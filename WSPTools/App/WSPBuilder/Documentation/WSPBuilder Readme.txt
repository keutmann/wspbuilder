...
---------------------
1.0.9.1118
WSPBuilder updated to support SharePoint 2010.
A manifest.config is used to control solution properties.
Visual Studio C# ProjectTemplate updated.


0.9.9.0813
CABLIB.DLL in the Deploy folder is now excluded from the wsp build.
Minor bugfix of CopyToGac function in WSPBuilder Extensions.


0.9.9.0728
A Reference folder under GAC and Bin is now supported for large dlls that do not need reflection on SafeControls and Permissions.
Recycles the Windows SharePoint Services Timer implemented on WSPBuilder Extensions.
...
CabLib.dll updated to new version.
Its now possible to include the Cablib.dll into the WSP package.

0.9.8.1029
Folder "Template" under the project folder is now supported. Both paths "..\project\12\template" and "..\project\template" are included.
